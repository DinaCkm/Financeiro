...
def rows_to_dicts(rows, header_idx):
    ...
    result = []
    for rel_idx, row in enumerate(rows[header_idx + 1:]):
        if not any(str(c).strip() for c in row):
            continue
        item = {'__rownum': header_idx + 2 + rel_idx}
...
def normalize_rows(raw_rows):
    ...
    for idx, raw in enumerate(raw_rows, start=1):
        source_row = raw.get('__rownum', idx)
        entry = {}

        # Mapear colunas
        for orig_key, orig_val in raw.items():
            if orig_key == '__rownum':
                continue
        ...
        if date_type.startswith('controle:'):
            skipped += 1
            continue
        if date_type == 'vazio' or date_type.startswith('invalido:'):
            skipped += 1
